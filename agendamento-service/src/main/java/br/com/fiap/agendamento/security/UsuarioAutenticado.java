 ...
 *
 * {@code pacienteId} e {@code profissionalId} viajam no token justamente para
 * que a restricao de escopo de um PACIENTE nao precise de ida ao banco. Sao
 * mutuamente exclusivos: e a check constraint ck_usuario_vinculo que garante.
 */
public record UsuarioAutenticado(
    ...
    public static UsuarioAutenticado de(Usuario usuario) {
        return new UsuarioAutenticado(usuario.getId(), usuario.getLogin(), usuario.getNome(),
                usuario.getRole(), usuario.getPacienteId(), usuario.getProfissionalId(),
                usuario.getSenha());
    }
    ...
    public static UsuarioAutenticado deToken(Long id, String login, String nome, Role role,
                                             Long pacienteId, Long profissionalId) {
        return new UsuarioAutenticado(id, login, nome, role, pacienteId, profissionalId, null);
    }
